...
// 정규화 Normalize
void FVector3D::Normalize()	// 해당 vector를 단위벡터로 만들어준다. -> 크기가 1인 벡터로 만드는 기능을 한다.
{
	float size = Length();
	if (0.f == size)
	{
...
FVector3D FVector3D::Normalize(const FVector3D& _v)	// 벡터가 들어오면 단위벡터를 만들어서 내보내준다.
{
	FVector3D result;
...
// TransforCoord : 행렬을 곱할때 w값을 1로 하냐
