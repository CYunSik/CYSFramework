...
bool CDevice::Init(HWND hWnd, unsigned int Width, unsigned int Height, bool WindowMode)
{
	...
	/*
		인자 설명
		1. 어떤 어댑터를 사용할것인지 지정하는데 여러개의 그래픽카드를 사용할때 메인으로 사용할 그래픽 카드를 넣어주는 곳이다.
	...
	*/
	...
	// SwapChain을 생성한다.
	// DESC가 붙은 구조체가 나온다.
	// 그럼 이 구조체를 채워서 넣어주면 그 설정대로 원하는대로 나온다.
	DXGI_SWAP_CHAIN_DESC SwapDesc = {};		// 초기화 해줘야 한다.
	...
	// __uuidof : 해당 클래스의 GUID를 얻어오는 연산이다. 디바이스 구분
	IDXGIDevice* DXGIDevice = nullptr;
	mDevice->QueryInterface(__uuidof(IDXGIDevice), (void**)&DXGIDevice);

	// 어댑터란? 그래픽카드(GPU)와 비디오 출력장치
	...
	// D24	: 깊이 24비트를 사용할 것이고, UNORM(0 ~ 1) 
