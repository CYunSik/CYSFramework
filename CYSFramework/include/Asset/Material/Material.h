...
protected:
	...
	// 이미지 텍스쳐 같이 쌍으로 들어가야할 데이터 : 샘플러
	ETextureSamplerType::Type mSamplerType = ETextureSamplerType::Linear;

public:
	// 텍스쳐 가져오기
	// CTexture 를 넣어주는 방법
	void AddTexture(class CTexture* Texture, int Register, int ShaderBufferType = EShaderBufferType::Pixel, int TextureIndex = 0);

	// Name으로 찾아와서 텍스쳐를 넣어주는 방법
	void AddTexture(const std::string& Name, int Register, int ShaderBufferType = EShaderBufferType::Pixel, int TextureIndex = 0);

	// 파일 이름으로 텍스쳐 매니져에서 로드 후 추가하기
	void AddTexture(const std::string& Name, const TCHAR* FileName, int Register, int ShaderBufferType = EShaderBufferType::Pixel, int TextureIndex = 0);
