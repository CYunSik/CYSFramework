...
bool CMeshManager::Init()
{
	...
	if (!CreateMesh("Box", BoxVtx, sizeof(FVertexColor), 8, D3D11_USAGE_DEFAULT, D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST, BoxIdx, sizeof(unsigned short), 36, DXGI_FORMAT_R16_UINT))
	{
		return false;
	}
	...
	if (!CreateMesh("CenterRect", CenterRect, sizeof(FVertexColor), 4, D3D11_USAGE_DEFAULT, D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST, RectIdx, sizeof(unsigned short), 6, DXGI_FORMAT_R16_UINT))
	{
		return false;
	}
	...
	if (!CreateMesh("FrameCenterRect", FrameCenterRect, sizeof(FVector3D), 4, D3D11_USAGE_DEFAULT, D3D10_PRIMITIVE_TOPOLOGY_LINESTRIP, FrameRectIdx, sizeof(unsigned short), 5, DXGI_FORMAT_R16_UINT))
	{
		return false;
	}
	...
	if (!CreateMesh("FrameSphere2D", Sphere2DPoint, sizeof(FVector3D), 37, D3D11_USAGE_DEFAULT, D3D10_PRIMITIVE_TOPOLOGY_LINESTRIP))
	{
		return false;
	}
	...
	if (!CreateMesh("LineUp2D", LineUp, sizeof(FVector3D), 2, D3D11_USAGE_DEFAULT, D3D10_PRIMITIVE_TOPOLOGY_LINESTRIP))
	{
		return false;
	}
	...
	if (!CreateMesh("LineRight2D", LineRight, sizeof(FVector3D), 2, D3D11_USAGE_DEFAULT, D3D10_PRIMITIVE_TOPOLOGY_LINESTRIP))
	{
		return false;
	}
	...
	if (!CreateMesh("CenterTexRect", CenterTexRect, sizeof(FVertexTexture), 4, D3D11_USAGE_DEFAULT, D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST, RectIdx, sizeof(unsigned short), 6, DXGI_FORMAT_R16_UINT))
	{
		return false;
	}
